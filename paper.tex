...
Africa. \aprcomment{.. to contributions from archaic hominin introgression in Africa?}
...
\sgcomment{explain in the main text where this estimate is from!}
...
\aprcomment{I think this is a fairly common term in statistics, right?}
allele frequency and linkage
disequilibrium (LD) statistics that are well suited for both low- and
high-coverage genomes \sgcomment{We computed one- and two-locus statistics whose expectation within and across populations can be computed efficiently and that are well suited for both low- and
...
Figure~\ref{fig:supp-possible-models}B), archaic hominin admixture
...
increase in $N_e$ between 100ka-1Ma (Figure \ref{fig:supp-iicr-sim}).
...
%\comment{These are pretty complicated ideas that are not explained in detail.
%Might be worth unpacking a bit more to make sure the reader understands}
