...
\begin{figure}[ht!]
    ...
    \caption{
        \textbf{Representative genetic diversity within Africa.}
        (A) Select populations from 1000 Genomes and ADRP illustrate diversity from
        ...
        British and Vindija Neanderthal individual (bold fonts). (B, C) PCA highlights the range of genetic divergence anchored 
        by western Africans, Nama, Gumuz and the British. (D) \texttt{ADMIXTURE}
...
following the out-of-Africa migration which was set to 1.5\% at 45kya (Supp. Methods). These constraints allowed us to integrate information from previous \sgcomment{genetic and archaelogical?} research,
...
\begin{table}[t!]
    ...
    \caption{
        ...
        genetic similarity. Bold text indicates migration rates above \sgcomment{XX} 
...
the early branches remain closely related, \sgcomment{and each branch contributes large amounts to all contemporary populations} (Figure 3). 
Because of this 
...
single-origin model, generate multi-modal curves recapitulating an inferred ancestral
increase in $N_e$ between 100kya-1Mya (Figure \ref{fig:supp-iicr-sim}). Thus the reduction in inverse coalescence rate 
is captured in our model, but is primarily explained by population structure in the best-fitting models \sgcomment{is that fair?}.     
None of the models perfectly match the exact timing of
the early fluctuations in $N_e$ observed in the data (Figure \ref{fig:supp-iicr-data-subset}). 
\sgcomment{Perhaps: These discrepancies are comparable to discrepancies observed in relate curves inferred 
for the same populations using different subsets of the data (see Figures \ref{ig:supp-iicr-data-subset} and \ref{ig:supp-iicr-data-subset})}
...
the Holocene, indicating either artifacts of data inference in real data or that additional free parameters are necessary to refine
these growth rates. \sgcomment{Perhaps: There are other discrepancies between odel }

\sgcomment{I am not sure I am happy with these interpretations 
-- let's discuss. I feel like this paragraph does not support our data well, and a likely culprit 
in explaining the differences is the inaccuracy of TMRCA estimates from real data.
Also, we would need a link to a supplementary figure!}
...
similar (supplementary text \sgcomment{X})\aprcomment{Will we still include this?} \sgcomment{yes!}, even if
