...
\comment{About title: "Contrasting archaic admixture and a weakly structured stem for human originas in Africa" Too long? Inconclusive?} \sgcomment{Disagree.}

\begin{abstract}
While it is now broadly accepted that \emph{Homo sapiens}
origined within Africa, considerable
...
well as variability in prior dating estimates.
Here we use linkage disequilibrium and
diversity-based statistics, optimized for rapid, complex demographic inference
to discriminate among such models \comment{\ldots to evaluate the fit of various models}.
...
differentiated ancestral \emph{Homo spp.}
\comment{maybe don't abbreviate; won't be understood outside of evolutionary biology as much}
...
similar.
Despite genetic similarity between these populations, an inferred 1--4\%\sgcomment{explain in the main text where this estimate is from!}
...
\comment{unpack more; explain why these are at odds}
Recently, synthetic \comment{synthetic doesn't feel quite right here, can delete}
...
\comment{I think a schematic illustrating models being tested would be extremely useful in the main text. This could be Fig S2, or even a new figure with a further simplified illustration o fthe general idea of the models being tested to help orient the reader.}
However, new
...
\citep{Henn2018-rf} (Figure~\ref{fig:supp-possible-models}).
Second, they
...
recruited populations of recent West African ancestry (Figure \ref{fig:1}C). 
\comment{What about the LWK?}
While ancient DNA from
...
\comment{Any point in citing this paper: https://www.nature.com/articles/s41586-022-04430-9 ?} \sgcomment{Yes, we can literally just cite it here. }
...
infer demographic models that explain more aspects of genetic diversity
\comment{this is a bit vague; make more detailed. e.g.: better fit patterns of LD and \ldots}
in more
...
stem. For each population, we computed low-order
\comment{what do you mean by low-order? not a commonly used phrase}
allele frequency and linkage
...
\begin{figure}[ht!]
    ...
    \caption[width=\textwidth]{
        ...
        \comment{How did you land on the best fit k? Show scree plot of CV errors, or otherwise justify choice}
    }
    \label{fig:1}
\end{figure}
...
reticulation (Figure~\ref{fig:supp-possible-models}B), archaic hominin admixture
...
history for the last 150ka was remarkably robust. The earliest divergence among
...
following the out-of-Africa migration which was set to 1.5\% at 45ka (Supp. Information). These constraints allowed us to integrate information from previous genetic and archaelogical research,
...
\begin{figure}[t!]
    ...
    \caption{
        ...
        Colors are used to distinguish overlapping branches and link to Figure \ref{fig:1}.}
    \label{fig:2}
\end{figure}
...
\begin{table}[t!]
    ...
    \begin{tabular}{lllrrrr}
        ...
        & & & 
            \textbf{Time (ka)} & \textbf{per generation} & \textbf{duration (ky)} \\
        \midrule
    ...
    \caption{
        ...
        groups 60ka. *Migration rates and durations are shown between branches ancestral to 1) Nama and East Africans
...
\comment{another supp schematic figure may be helpful}
...
proposed archaic hominin admixture models (Figure~\ref{fig:supp-possible-models}C) as
...
Because of this 
relatedness, only $1\%$ to $4\%$ of genetic differentiation among contemporary populations can be traced back
...
(for example, compare with Figure~1 in
...
increase in $N_e$ between 100ka-1Ma (Figure \ref{fig:supp-iicr-sim}). Whereas the single-origin model achieves this by
...
\comment{These are pretty complicated ideas that are not explained in detail. Might be worth unpacking a bit more to make sure the reader understands}
...
divergence, isolation and then a merger event (i.e. a ``reticulation'' \comment{define at the first mention})
...
Stem 1S \comment{where is this illustrated/labeled?}
from southern Africa moved northward thereby encountering the Stem 2,
...
\comment{very specific... might be worth expanding a bit}
...
a weakly-structured-stem model within Africa. \comment{Reviewers may suggest this demonstrated here}
...
(see also Section \ref{morpho_diff}).
...
available fossils, some are very similar to contemporary humans
(e.g., from Omo Kibish, Ethiopia \citep{Day1969-rh,Vidal2022-qe}),
others are similar in some features but not others
(e.g., from Jebel Irhoud, Morocco \citep{Hublin2017-cq,Richter2017-zu}),
and others are very different
(e.g., from Dinaledi, South Africa \citep{Berger2015-bq,Dirks2017-uk}) in morphology.
